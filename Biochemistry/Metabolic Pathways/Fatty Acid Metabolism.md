#Metabolism 

## Catabolism

$\beta$ oxidation

FAs provide way more energy than glucose, because they are less oxidized than glucose.

#### Lipid droplet

Storage of lipids
the membrane is a phospholipid monolayer

Hormones

**Lipases** - enzymes that split FAs from glycerol; hydrolysis reaction. ==where happens, if inside is hydrophobic?==
**Serum albumin** - protein that transports FAs through the blood; ~50% of blood.

Glycerol gets converted to dihydroxyacetone phosphate (DHAP) and enters glycolysis (LINK).

#### FA degradation

1.
In cytoplasm
uses ATP
make thioester to CoA
**Adenylyl transfer** - transfer of the Pi-Adenosine group.
Adenylyl is a better leaving group than Pi

2.
Carnitine shuttle - transfers FAs into mitochondria.

#### Beta oxidation

In mitochondrial matrix
oxidizing beta position of the FA
stepwise removal of 2-carbon units in the form of Acetyl-CoA
Acetyl-CoA goes into the TCA cycle
the e^- from the C-C bonds get transferred onto NADH and enter ETC

1.
oxidize to introduce a double bond

2.
hydrate the double bond

3.
oxidize to a ketone

4.
cleave an acetyl group

#### Monounsaturated Fatty Acids

Special enzyme makes sure the double bond is at the correct position.
Then the cycle continues.

#### Polyunsaturated Fatty Acids

Doesn't fit in the enzymes.

2,4-dienoyl-CoA reductase converts a 2,4-dienoyl into 3-enoyl.
Then, see [[#Monounsaturated Fatty Acids]].

#### Odd-numbered Fatty Acids

Propionul-CoA left after beta odidation.
Me group added, then placed at the end.
Creates a C4 compound.
Then beta oxidation again.

Vitamin B12 - coenzyme for exchanges of group between adjacent carbons; has $\ce{Co^{3+}}$
Cobalt bond is susceptible to homolytic cleavage.


## Anabolism

Entirely different pathway compared to catabolism.
Done in the cytoplasm.

FA synthase - single polypeptide that does almost all of lipid anabolism on mammals.

ACP - acyl-carrier protein; intermediates stay attached to it.

NADP+ is used.

Acetyl-CoA carboxylase - makes malonyl-CoA (biotin coenzyme)

## Regulation

Acetyl-CoA carboxylase is regulated in anabolism.
Phosphorylated enzyme is inactive.
Deactivation - AMP stimulates, ATP inhibits
Activation - insulin stimulates, glucagon and epinephrine inhibit
