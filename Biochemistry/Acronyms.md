
| Acronym     | Explanation                                 | Comment                                                                              |
| ----------- | ------------------------------------------- | ------------------------------------------------------------------------------------ |
| $\ce{Pi}$   | Phosphate/phosphoryl                        | $\ce{PO4^{3-}}$                                                                      |
| $\ce{PPi}$  | Pyrophosphate                               | $\ce{[O3P-O-PO3]^{4-}}$; a diphosphate group                                         |
| $\ce{ATP}$  | Adenosine triphosphate                      | Energy currency of the cell                                                          |
| $\ce{ADP}$  | Adenosine diphosphate                       | ATP molecule depleted of energy                                                      |
| $\ce{AMP}$  | Adenosine monophosphate                     | Molecule that is used to supress processes that normally require ATP                 |
| $\ce{NAD}$  | Nicotinamide adenine dinucleotide           | $e^-$ carrier; used in catabolism                                                    |
| $\ce{FAD}$  | Flavin adenine dinucleotide                 | $e^-$ carrier; used in catabolism ==what else?                                       |
| $\ce{NADP}$ | Nicotinamide adenine dinucleotide phosphate | $e^-$ carrier; used in anabolism                                                     |
| DHAP        | Dihydroxyacetone phosphate                  | Intermediate in glycolysis                                                           |
| $\ce{Q}$    | Ubiquinone                                  | $e^-$ carrier; used in catabolism; $\ce{QH2}$ can diffuse in cell membranes and move |
| TCA Cycle   | Tricarboxylic acid cycle                    | 2nd step in catabolism; happens in the mitochondrial matrix                          |
| ETC         | Electron Transport Chain                    | 3rd step in catabolism; happens between the membranes of a mitochondrion             |
| PLP         | Pyridoxal phosphate                         | Cofactor for aminotransferases                                                       |
| ACP         | Acyl-carrier Protein                        | Intermediates in the lipid anabolism stay attached to it                             |


